...
if page == "🏠 总览大屏":
    ...
    with col4:
        ...
        <div class="metric-card">
            ...
            <div class="metric-label">🌿 修复面积/km²</div>
    ...
    with col_left:
        with st.container():
            ...
            fig = go.Figure(data=[go.Pie(
                ...
                values=[1, 6, 1],
                hole=0.65,
                marker_colors=["#00ffff", "#44aaff", "#ffaa44"],
    ...
    with col_right:
        ...
        ranking = water_data.sort_values(by="溶解氧(mg/L)", ascending=False).head(8)
        for i, row in ranking.iterrows():
            st.markdown(f"""
            <div class="rank-item">
                <span class="rank-number">{ranking.index.get_loc(i) + 1}</span> {row['监测点']}
                <div>溶解氧: {row['溶解氧(mg/L)']} mg/L <span style="color:#00ffff;">{row['水质等级']}</span></div>
            </div>
            """, unsafe_allow_html=True)
...
# ============ 鱼类修复页面 ============
    ...
    with col2:
        ...
        st.progress(value=60, text="鱼类栖息地修复工程")
        st.progress(value=45, text="产卵场生态重建")
